...
def vis_box(x, img, path, np_array, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        cv2.putText(img, 'pothole', (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
    cv2.imshow('',img)
    # 将 NumPy 数组保存为图片
...
def detect(source, save_img=True):
    weights = 'checkpoint/detection.pt'
    view_img = True
    save_txt = False
    imgsz = 640
    device = ''
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        np_array, imgd = vis_box(xyxy, im0, path, np_array, label=label, color=colors[int(cls)],
                                                 line_thickness=3)
            else:
                imgd = im0
        ...
        for imgcur in imgs:
            ...
            for annts in np_array:
                if clss[annts[5]] in out_cls:
                    annts_list.append([annts[0], annts[1], annts[2], annts[3]])

            if len(annts_list) > 0:
                str1 = ''
                for ls in annts_list:
                    for i in ls:
                        str1 += str(i) + ','
                    str1 = str1[:-1] + '|'

                txt = imgcur + '|' + str1 + '\n'
                with open('dataset/pothole_label.txt', 'a+') as writer:
                    writer.write(txt)
